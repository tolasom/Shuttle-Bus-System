...
public class Customer_Schedule_Generation_Imp implements Customer_Schedule_Generation_Dao {
    ...
    public String customer_schedule_generation(Session session,Customer_Booking cb) throws ParseException {
        ...
        try {
            ...
            Boolean check_assign = true; // Check whether we can assign this passenger to existing schedule or not
            for (int i = 0; i < schedule.size(); i++) {
                if (schedule.get(i).getRemaining_seat() >= cb.getNumber_of_seat()) {
                    ...
                    Query query = session.createQuery("update Schedule_Master set number_booking=:num_booking, " +
                            "remaining_seat=:remain_seat, number_customer=:number_customer" +
                            " where id = :id");
